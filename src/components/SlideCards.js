      ...
      {projects.map((project, index) => (
        <div key={index} className="w-96 border border-black flex flex-col place-content-between shrink-0 rounded overflow-hidden shadow-lg mx-2 bg-black/20">
          ...
          <div className="px-6 flex flex-col justify-centre text-center text-white">
            ...
            <a href={project.codeLink} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-l from-red-900 to-black/70 hover:bg-gradient-to-l hover:from-black/70 hover:to-red-800 inline-block bg-pink-100 rounded-full px-3 py-1 text-sm font-semibold mr-2 m-2">
              Code
            </a>
            <a href={project.siteLink} target="_blank" rel="noopener noreferrer" className="inline-block bg-gradient-to-l from-green-900 to-black/70 hover:bg-gradient-to-l hover:from-black/70 hover:to-green-800 rounded-full px-3 py-1 text-sm font-semibold m-2">
              Live Site
            </a>
          </div>
        </div>
